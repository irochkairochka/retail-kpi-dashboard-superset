...
Фильтр: только East-регион + Technology.  
    - Выручка резко упала с 2.26M до 158K.  
    - Orders Count снизился с 4922 до 261.  
    - AOV вырос с 459 до 607 → товар дорогой, но спроса мало.  
...
![KPI Dashboard](./screenshots/kpi_dashboard.jpg)  
...
### Пример фильтрации по срезам  

![Фильтрация срезов](./screenshots/srezy_kpi_dashboard.jpg)  

**На скрине**: Выбран только East-регион, видно как меняются метрики.  

### Детальная выборка (East + Technology)  

![Детальный анализ](./screenshots/klick_kpi_dashboard.jpg)  

**На скрине**: Комбинированный фильтр (регион East + категория Technology) показывает узкий срез данных. AOV растёт (дорогие товары), но объём падает (мало спроса).