...
- Общая выручка (2.26 млн руб. — полный период)
- Количество чеков (4922 заказа)
- Средняя сумма чека (459.48 руб.)
...
retail-kpi-dashboard-superset/
├── data/
│   └── superstore.csv                 # исходный датасет (9994 записей)
├── reports/
│   └── retail_kpi_dashboard.pbix      # файл Power BI (главный артефакт)
├── screenshots/
│   ├── kpi_dashboard.jpg              # общий вид дашборда (полный период)
│   ├── srezy_kpi_dashboard.jpg        # пример работы срезов (фильтрация)
│   └── klick_kpi_dashboard.jpg        # пример детальной выборки (East + Technology)
└── README.md                          # этот файл
...
Фильтр: только East-регион + Technology.
    ...
    - AOV вырос с 459 до 607 руб. → товар дорогой, но спроса мало.
